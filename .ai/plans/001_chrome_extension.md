...
## Summary
Build the first working TabTidy Chrome extension as a vanilla TypeScript Manifest V3 project. The toolbar action will immediately close all non-pinned tabs in the current window, save their details into extension storage, and ship a simple business-blue history page that shows saved tabs grouped by local day with newest entries first.
...
4. Implement the action flow: query tabs in the current window, exclude pinned tabs, capture `{ url, title, closedAt, dayKey }` for each tab being closed, append those records to a single `chrome.storage.local` collection, then close only the non-pinned tabs.
...
- Use Chrome MV3 APIs only: `chrome.action.onClicked`, `chrome.tabs.query`, `chrome.tabs.remove`, and `chrome.storage.local`.

## Test Plan
- Unit-test record normalization, append behavior, day grouping, and newest-first sorting.
- Manually verify that clicking the action closes only non-pinned tabs in the current window and leaves pinned tabs untouched.
