...
5. Implement the history page to load stored records, group them by tidy run using their shared `closedAt` timestamp, sort newest run first, render session cards without an extra outer container, show the TabTidy icon at the right edge of the grouped title/subtitle header content, keep each saved entry on one line with a URL that truncates inside the card after the title, and let the user delete a saved entry immediately with a simple `×` button.
